...
class ControllerExtensionPaymentECPayInvoice extends Controller 
{
	...
	{
		$this->load->language('extension/payment/ecpayinvoice');
		...
		$data['breadcrumbs'][] = array(
    		...
    		'href' => $this->url->link('extension/payment/ecpayinvoice', 'token=' . $this->session->data['token'], true)
        	); 
		...
		$data['action'] = $this->url->link('extension/payment/ecpayinvoice', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('extension/payment/ecpayinvoice.tpl', $data));
	}
	
	private function validate() 
	{
		if (!$this->user->hasPermission('modify', 'extension/payment/ecpayinvoice')) {
